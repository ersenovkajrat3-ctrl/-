...
(function (global) {
  ...
  const EURO_NICKS = ['Волеро', 'Астра', 'Кастелло', 'Пантера', 'Виктория', 'Атлас', 'Корона', 'Аврора', 'Спарта', 'Феникс', 'Кондор', 'Легион',
    ...
    'Сирокко', 'Тандем', 'Гладио', 'Нордик', 'Пирамида', 'Кристалл', 'Ротор', 'Юпитер', 'Магнум', 'Ястреб'];
    ...
    for (const c of EURO_POOL) {
      for (const city of c.cities) {
        ...
        out.push({
          ...
          // сила приведена к той же шкале, что и составы наших клубов: иначе европейцы
          // оказывались заведомо слабее и лига забирала все три трофея каждый сезон
          power: U.clamp(Math.round(c.power * 0.90 + rng.range(-5, 5) + (out.length % 3)), 45, 92),
          foreign: true,
        });
      }
    }
