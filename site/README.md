...
Вместо статичной заглушки в шапке работает канвас: кадр разбирается на
6480 частиц, они разлетаются волной по диагонали, окрашиваясь в индиго, и
собираются в следующий кадр. Чередуются AI-работы и живая съёмка.

Кадры лежат в `images/hero/`, список и подписи — в массиве `SHOTS` внутри
`index.html`. Чтобы поменять кадр, положите файл 240x300 и поправьте строку:
